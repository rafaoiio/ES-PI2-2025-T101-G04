...
export class DashboardController {
  ...
  async getMetrics(@Request() req, @Res() res: Response) {
    try {
      ...
      console.log(
        '[Dashboard Controller] idProfessor recebido (raw):',
        idProfessorRaw,
      );
      console.log('[Dashboard Controller] Tipo:', typeof idProfessorRaw);
      console.log(
        '[Dashboard Controller] req.user completo:',
        JSON.stringify(req.user, null, 2),
      );

      if (!idProfessorRaw) {
        console.error(
          '[Dashboard Controller] idProfessor não encontrado em req.user',
        );
      ...
      console.log(
        '[Dashboard Controller] idProfessor convertido:',
        idProfessor,
      );
      console.log(
        '[Dashboard Controller] Tipo após conversão:',
        typeof idProfessor,
      );

      if (isNaN(idProfessor)) {
        console.error(
          '[Dashboard Controller] idProfessor não é um número válido:',
          idProfessorRaw,
        );
